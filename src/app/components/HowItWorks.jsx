...
export default function HowItWorks() {
  ...
  return (
    <div className="w-full flex flex-col items-center py-12">
        ...
        {steps.map((step, index) => (
          ...
          >
            <img
              ...
              className="absolute inset-0 w-full h-97 object-cover rounded-4xl -translate-y-3 translate-x-6 -z-10 img-bottom filter brightness-0"
            />

            <div className="relative overflow-hidden rounded-3xl shadow-md clip-bottom">
              <div className="bg-white border border-[#f4f4f4] rounded-2xl p-3 flex flex-col items-center text-center h-[400px] clip-bottom">
                <div className="bg-black border border-[#e6e6e6] rounded-2xl shadow-inner p-6 flex flex-col items-center text-center h-full clip-bottom">

                  <h3 className="lg:text-[1.5rem] leading-7 mb-1 mt-6 text-white">{step.title}</h3>
                  <p className="text-sm text-white mb-6 mt-5">{step.desc}</p>
                  <div
                    className={`w-14 h-14 flex items-center justify-center rounded-full ${step.color} text-black text-2xl font-bold mt-3`}
                  >
                    {step.id}
                  </div>
                </div>
      ...
      <button
        ref={buttonRef}
        className="mt-12 text-xs md:text-base px-8 py-4 rounded-full bg-black text-white flex items-center gap-2 shadow-lg text-center"
      >
